...
def yolo_loss(predict,
    ...
    Args:
        ...
        labels : 2-D list [object_num, 5] (xcenter (Absolute coordinate), ycenter(absolute coordintate), w(Absolute coordinate), h(Absolute coordinate), class_num)
        object_num :each_object number in image
        num_classes :number of classes
        boxes_per_cell : number of prediction boxes per each cell
        cell_size : each cell_size
        ...
        noojbect_scale : coefficient for noobject loss
    ...
    # find best box mask
    I = iou_predict_truth
    max_I = tf.reduce_max(I, 2, keepims=True)
    ...
    object_exists_cell_i, object_exists_cell_j = int(cell_size * ycenter / input_height) , int(cell_size, cell_size, input_width)
    object_exists_cell[object_exists_cell_i][object_exists_cell_j] = 1
    
    #set coord_loss
    coord_loss = (tf.nn.l2loss(object_exists_cell * best_box_mask * (pred_xcenter - xcenter) / (input_width/cell_size)) +
                  tf.nn.l2loss(object_exists_cell * best_box_mask * (pred_ycenter - ycenter) / (input_height/cell_size)) +
                  tf.nn.l2loss(object_exists_cell * best_box_mask * (pred_sqrt_w - sqrt_w) / input_width) +
                  tf.nn.l2loss(object_exists_cell * best_box_mask * (pred_sqrt_h - sqrt_h) / input_height) \
                  * coord_scale
                  )
    ...
    # noobject_loss
    noobect_loss = tf.nn.l2_loss(object_exists_cell * best_box_mask * pred_C) * noobject_scale
    
    # class loss
    class_loss = tf.nn.l2_loss(object_exists_cell * (pred_P - P)) * class_scale
    
    # sum every loss
    total_loss = coord_loss + object_loss + noobect_loss + class_loss
    
    return total_loss, coord_loss, object_loss, noobect_loss, class_loss
